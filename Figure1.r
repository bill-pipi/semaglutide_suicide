...
workdir <- "."
datafile <- file.path(workdir,"Figure_obesity_no.csv")
...
tabletext <- cbind(c("Population",data$blah),
                    c("Semaglutide\ngroup\n",data$case_group),
                    c("Non-GLP1R agonist\nanti-obesity medications group\n",data$control_group),
                   c("        HR (95% CI)",data$CI)
)

pdf(file.path(workdir,"Figure1.pdf"),  onefile=FALSE, width=10, height=5.5)
...
forestplot(labeltext=tabletext, 
           ...
           title ="Incident suicidal ideations in patients with overweight or obesity \nwithin 6-month time window\n(comparison between propensity-score matched groups)", 
